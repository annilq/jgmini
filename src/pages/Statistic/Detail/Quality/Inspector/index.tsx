...
function Main(props) {
  ...
  const searchArr = [
    {
      name: 'inspectorName',
      component: <Input placeholder="检查人" />,
    },
  ]

  const rowKey = "checkerId"
  ...
  const { data = { list: [] }, status } = useFetch<JGListData>(service.listByChecker, params);
  ...
  return (
    <>
      ...
      <Layer
        type="drawer"
        width="80vw"
        title={<LayerHeader data={{ name: `${title}-检查人` }} onClose={() => setVisible(false)} />}
        visible={visible}

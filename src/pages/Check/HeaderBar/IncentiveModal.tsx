...
function IncentiveModal(props: IProps) {
  const { visible, dispatch, project, onClose, data, loading } = props;
  const formCode = 'Incentive';
  ...
  function submit(values) {
    ...
    // 质量1安全2
    dispatch({
      type: 'qualityIncentive/addRemote',
      payload: {
        ...values,
...
export default connect(({ jgTableModel, project, loading }) => ({
  ...
  loading: loading.effects['qualityIncentive/addRemote']
}))(IncentiveModal);

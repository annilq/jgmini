...
@connect(({ projectProcessReport, loading }) => ({
  projectProcessReport,
  tableLoading: loading.effects['projectProcessReport/listRemote'] || false
}))
class Index extends PureComponent {
    ...
    // },
    {
      ...
      component: (
        <JgSelect
          ...
          data={[{ label: "发布", value: "publish" }, { label: "草稿", value: "normal" }]} />
      ),
    }
  ];
  ...
  search = params => {
    ...
    dispatch({ type: 'projectProcessReport/listRemote', payload: params });
  };

  // 重置
  reset = () => {
    const { dispatch } = this.props;
    dispatch({ type: 'projectProcessReport/listRemote' });
  };

  // 分页、排序、筛选变化时触发
  handleStandardTableChange = pagination => {
    const { dispatch } = this.props;
    dispatch({ type: 'projectProcessReport/pageRemote', payload: { ...pagination } });
  };

  // 删除
  remove = record => {
    const { dispatch } = this.props;
    dispatch({ type: 'projectProcessReport/removeRemote', payload: { id: record.id } });
  };

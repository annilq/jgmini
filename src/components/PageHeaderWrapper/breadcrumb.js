 ...
 * Convert parameters into breadcrumbs
 */
export const conversionBreadcrumbString = props => {
  const { routerLocation, breadcrumbNameMap } = getBreadcrumbProps(props);
  let path = '首页  /  ';
  if (routerLocation && routerLocation.pathname) {
    const list = conversionFromLocation(routerLocation, breadcrumbNameMap, props);
    if (list && list.length > 0) {
      for (let i = 0; i < list.length; i++) {
        path = path + list[i].breadcrumbName + '  /  ';
      }
    }
  }
  return path;
